...
def run_experiment(max_budget: int):
    ...
    intersvarm_estimator = InterSVARM(N=N, order=order, interaction_type=interaction_index, top_order=RUN_TOP_ORDER)

    # SHAP-IQ estimator for all three indices
    shapiq_estimator = SHAPIQEstimator(N=N, order=order, interaction_type=interaction_index, top_order=RUN_TOP_ORDER)

    # get baseline estimator
    if interaction_index in ['SII', 'STI']:
        # for SII and STI we use the permutation sampling estimator
        baseline_estimator = PermutationSampling(N=N, order=order, interaction_type=interaction_index, top_order=RUN_TOP_ORDER)
    else:
        # for SFI we use the regression estimator
        baseline_estimator = RegressionEstimator(N=N, max_order=order)
    ...
    RESULTS = run_top_order_experiment(
        ...
        game_list=game_list,
        intersvarm_estimator=intersvarm_estimator,
...
if __name__ == "__main__":
    ...
    interaction_index = "FSI" if has_no_param else run_parameters[3]
    RUN_TOP_ORDER = True if has_no_param else run_parameters[4] == "True"
    order = 2 if has_no_param else int(run_parameters[5])
    NUMBER_OF_RUNS = 50 if has_no_param else int(run_parameters[6])
    MAX_BUDGET = 2 ** data_n if has_no_param else int(run_parameters[7])

    print("selected parameters:", data_folder, data_n, interaction_index, RUN_TOP_ORDER, order, NUMBER_OF_RUNS, MAX_BUDGET)

    run_experiment(max_budget=MAX_BUDGET)
    # data_id = 1; order_plot = 2; import matplotlib.pyplot as plt; plt.hist(all_gt_values[data_id][order_plot].flatten()); plt.show()
